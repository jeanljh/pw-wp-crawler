...
test.describe('Test Suite', async () => {
  ...
  test('Install and activate the crawler plugin', async ({ page, plugins }) => {
    ...
    await plugins.installStatus.waitFor()
  })

  test('Check UI of the crawler admin panel', async ({ page, adminPanel }) => {
    await page.goto('/wp-admin/tools.php?page=wpcrawler_admin')
    await expect.soft(adminPanel.headers).toHaveText(['WP Crawler admin panel', 'Website structure', 'Crawl controls'])
    expect
      .soft((await adminPanel.pageslist.allTextContents()).every((t) => t.startsWith('http://web.local')))
      .toBeTruthy()
    await expect.soft(adminPanel.crawlButton).toHaveText('Crawl Website')
    const regExp =
      /^Automatic crawls are scheduled\. Next one is (\b(?:Sun|Mon|Tue|Wed|Thu|Fri|Sat)\b \b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b \d{1,2} \d{4} \d{2}:\d{2}:\d{2} ([A-Z]{3})[+-]\d{4} \((.+?) Time\)).*$/
    await expect(adminPanel.crawlStatus).toHaveText(regExp)
  })

  test('Manually launch the crawler', async ({ page, adminPanel }) => {
    await page.goto('/wp-admin/tools.php?page=wpcrawler_admin')
    ...
    await expect.soft(adminPanel.getUrl(baseUrl)).toHaveCount(1)
  })
  ...
  test('Manually crawl to update the website', async ({ page, posts, adminPanel }) => {
    ...
    await page.route('**/admin-ajax.php', (r) => r.continue()) // intercept second "admin-ajax" request and continue to next one
    await page.waitForResponse((r) => r.url().endsWith('admin-ajax.php') && r.status() === 200)
    await expect.soft(adminPanel.getUrl(baseUrl + '/new-post/')).toHaveCount(1, { timeout: 20000 })
    ...
    await posts.updateButton.click()

    await page.goto('/wp-admin/tools.php?page=wpcrawler_admin')
    await adminPanel.crawlButton.click()
    await page.route('**/admin-ajax.php', (r) => r.continue()) // intercept first "admin-ajax" request and continue
    await page.route('**/admin-ajax.php', (r) => r.continue()) // intercept second "admin-ajax" request and continue
    await page.waitForResponse((r) => r.url().endsWith('admin-ajax.php') && r.status() === 200)
    await expect.soft(adminPanel.getUrl(baseUrl + '/edit-post/')).toHaveCount(1, { timeout: 20000 })

    await page.goto('/wp-admin/edit.php')
    await posts.postColumn.first().hover()
    await posts.trashLink.first().click()
    ...
    await page.route('**/admin-ajax.php', (r) => r.continue()) // intercept second "admin-ajax" request and continue
    await page.waitForResponse((r) => r.url().endsWith('admin-ajax.php') && r.status() === 200)
    await expect(adminPanel.getUrl(baseUrl + '/edit-post/')).not.toBeVisible()
  })
